...
class TopshopSpider(scrapy.Spider):
    ...
    def start_requests(self):
        url = 'https://www.topshop.com/en/tsuk/'
    ...
    def category_collection(self, response):
        cat_urls_matches = response.xpath(
            './/div[@class="HeaderTopshop-navigation"]//a[contains(@href, "/tsuk/category/")]'
        )
        for cat_urls_match in cat_urls_matches:
            ...
            yield scrapy.Request(
                url=f'https://www.topshop.com{cat_url.extract_first()}',
                callback=self.product_collection,
                meta={
                    'cat_name': cat_name.extract_first(),
                    'cat_url': f'https://www.topshop.com{cat_url.extract_first()}'
                }
            )
